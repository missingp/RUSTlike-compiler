...
{
	int res = 1;
	// 创建函数表：funcName函数名，funcPara函数参数,funcRet
	SemanticSymbol funcName = symbolList[symbolList.size() - 4];
	SemanticSymbol funcPara = symbolList[symbolList.size() - 2];
	SemanticSymbol funcRet = symbolList[symbolList.size() - 4];
	if (production_right.size() == 7)
	{
		funcName = symbolList[symbolList.size() - 7];
		funcPara = symbolList[symbolList.size() - 5];
		funcRet = symbolList[symbolList.size() - 1];
	}

	// 判断函数名是否重定义
	res = ProcessError(funcName, nullptr, nullptr, FUNCTION_REDEFINED, pos);
	cout << "funcName" << funcName.value << endl;
	...
	// 返回值
	//cout << "Ret value :" << funcRet.value << endl;
	IdentifierInfo return_value({ IdentifierInfo::ReturnValue, funcRet.value, funcPara.value });
	//printf("nextQ funHead:%d\n", nextQuaternionIndex);
...
int SemanticAnalysis::TranslateVarDeclAssign(const string production_left, const vector<string> production_right, int pos)
{
	...
	if (production_right.size() == 7)
	{
		idtf = symbolList[symbolList.size() - 7];
	}
	int tbIndex = -1, tbIndexIndex = -1;
	cout << "vardeclass:" << idtf.value << endl;
...
int SemanticAnalysis::TranslateExp(const string production_left, const vector<string> production_right, int pos)
{
	if (production_right.size() == 1)
	{
		...
		//cout << "ASE:" << add_sub_exp.value <<endl;
		PopSymbolList(1);
...
int SemanticAnalysis::TranslateReturnStmt(const string production_left, const vector<string> production_right, int pos)
{
	...
	for (auto& symbol : symbol_table.symbols) {
		if (symbol.category == IdentifierInfo::ReturnValue) {
			return_type = symbol.specifierType;
			break;
		}
	}
	cout << "ret ty: "<<return_type << endl;

	if (right_size == 2)// return ;
	{
		...
		if (return_type != "void")
		{
			//cout << "缺少返回值" << endl;
			int invalid = 0;
			res = ProcessError(SemanticSymbol("void", "return", 0, 0), &invalid, &invalid, NO_RETURN, pos);
		}
		PopSymbolList(right_size);
	...
	else
	{
		...
		cout << "exp type: " << exp.value << endl;
		if (!isAllDigits(exp.value)) {
			res = ProcessError(exp, nullptr, nullptr, NO_RETURN, pos);
		}
		string ret_name = symbol_table.tableName + "_return_value";
		quaternionList.push_back({ nextQuaternionIndex++, "=", exp.value, "-", ret_name });
		quaternionList.push_back({ nextQuaternionIndex++, "return", ret_name, "-", symbol_table.tableName });
...
int SemanticAnalysis::TranslateType(const string production_left, const vector<string> production_right, int pos) {
	...
	// Type ::= i32
	if (rightSize == 1 && production_right[0] == "i32") {
		//cout << "I32" << endl;
		symbolList.emplace_back(production_left, "i32", -1, -1);
	}
	// Type ::= [ Type ; <INT> ]
	else if (rightSize == 4 && production_right[0] == "[" && production_right[2] == ";" && production_right[3] == "<INT>") {
		SemanticSymbol typeSym = symbolList[symbolList.size() - 3];
		SemanticSymbol intSym = symbolList[symbolList.size() - 1];
		string temp = "array[" + intSym.value + "] of " + typeSym.value;

		PopSymbolList(4);
		symbolList.emplace_back(production_left, temp, -1, -1);
	}
	// Type ::= ( TupleTypeInner )
	else if (rightSize == 3 && production_right[0] == "(" && production_right[2] == ")") {
		SemanticSymbol tupleInner = symbolList[symbolList.size() - 2];
		string temp = "tuple(" + tupleInner.value + ")";
	...
	// Type ::= & mut Type
		...
		SemanticSymbol typeSym = symbolList.back();
		string temp = "&mut " + typeSym.value;
